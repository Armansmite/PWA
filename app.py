...
# ---------- EXPORT / IMPORT (per‑user Supabase) ----------
...
def export_config():
    ...
    token_row = supabase_get("user_token", user_id)
    token_json = token_row.get("token_json") if token_row else None
    return jsonify({
        'settings': settings,
...
def import_config():
    ...
    data = request.get_json()
    if 'settings' in data:
        supabase_upsert("user_settings", user_id, {"settings": data['settings']})
    if 'credentials_json' in data and data['credentials_json']:
        supabase_upsert("user_token", user_id, {"token_json": data['credentials_json']})
    return jsonify({'status': 'imported', 'settings': data.get('settings', {})})
