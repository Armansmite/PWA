...
def handle_token():
    ...
    if request.method == 'POST':
        # Called from Auth tab after OAuth, or from Files tab upload
        token_json = request.get_json().get("token_json")
        if not token_json:
            # Maybe file upload via multipart?
...
# ---------- AUTH UPLOAD / AUTHENTICATE (unchanged logic) ----------
@app.route('/api/auth/upload_client_secret', methods=['POST'])
def upload_client_secret():
    # This still works with in-memory flow, but the resulting token is saved via /api/auth/authenticate
    ...
    try:
        ...
        # Store flow in memory (bad, but Render won't restart mid‑session)
